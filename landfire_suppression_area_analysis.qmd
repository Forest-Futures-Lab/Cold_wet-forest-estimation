...
2.  We then use 2020 BPS layer to filter the forest cells in fire regime group (4A, 4B, 5A, or 5B) and fire return interval is longer than 80 years.
    ...
    writeCDF(long.interval.veg, paste0(data.directory,"long.interval.forested.gridcells.nc"))
...
```{r}
vcc.filtered=ifel(US_230VCC==1|US_230VCC==2,1,NA)
long.interval.veg.final=ifel(!is.na(vcc.filtered),long.interval.veg,NA)
writeCDF(long.interval.veg.final, paste0(data.directory,"long.interval.forested.gridcells.low.departure.nc"))
```
...
long.interval.veg = rast(paste0(data.directory,"long.interval.forested.gridcells.low.departure.nc"))
